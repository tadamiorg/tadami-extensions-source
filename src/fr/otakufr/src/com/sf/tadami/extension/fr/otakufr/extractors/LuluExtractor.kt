...
class LuluExtractor(private val client: OkHttpClient, private val headers: Headers) {
    ...
    fun videosFromUrl(url: String): List<StreamSource> {
        ...
        val videoHeaders = baseHeaders.newBuilder()
            .set("Origin","https://luluvdo.com")
            .set("Referer","https://luluvdo.com/")
            .build()

        val doc = client.newCall(GET(url,baseHeaders)).execute().asJsoup()
        val masterUrl = extractM3u8Url(doc) ?: return emptyList()
        return PlaylistUtils(client,videoHeaders).extractFromHls(masterUrl,videoNameGen = { qual -> "LuluVdo - $qual" }).map {
            it.copy(server = "LuluVdo")
        }
    }
}