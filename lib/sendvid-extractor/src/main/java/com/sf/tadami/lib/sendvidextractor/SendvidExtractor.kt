    ...
    fun videosFromUrl(url: String, prefix: String = "", suffix: String = ""): List<StreamSource> {
        ...
        when{
            masterUrl.contains(".m3u8") -> {
                return  playlistUtils.extractFromHls(masterUrl, url, videoNameGen = { prefix + "Sendvid - $it" }).map {
                    it.copy(server = "Sendvid")
                }
            }
            masterUrl.contains(".mp4") -> {
                return listOf(StreamSource(url = masterUrl, fullName = "$prefix Sendvid", server = "Sendvid", headers = headers))
            }
            else -> {
