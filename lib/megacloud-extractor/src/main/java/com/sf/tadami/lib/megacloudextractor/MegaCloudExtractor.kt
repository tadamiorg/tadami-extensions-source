...
class MegaCloudExtractor(
    ...
    private fun getKey(type: String): List<List<Int>> = runLocked {
        ...
        json.decodeFromString<List<List<Int>>>(
            runBlocking {
                dataStore.getDataStoreValues()[stringPreferencesKey(PREF_KEY_KEY + type)]?: PREF_KEY_DEFAULT
            }
        )
    }
    ...
    private fun updateKey(type: String) {
        ...
        runBlocking {
            dataStore.editPreference(encoded, stringPreferencesKey(PREF_KEY_KEY + type))
        }
    }
